...
void checkandgetargumentssub(int argc, char **argv, int *nx, int *ny, int *xprocs, int *yprocs, int *nxp, int *nyp, int *subnum, int *ioworker, int *lowerrank, int *upperrank, int *barrier, int size, int rank){
  ...
  if ( argc != 8 ){
    ...
    if(rank == 0){
      printf("usage: %s filename nx ny xprocs yprocs barrier numio\n", argv[0] );
      printf("This application expects you to provide the input file name, the size of the input data set (nx*ny), the number of processes you want in each dimension (xproc and yproc), and whether to use a barrier before timing (barrier, 0 is no barrier, 1 uses a barrier), the number of I/O workers you want (numio)\n");
    }
    MPI_Finalize();
    exit(-1);
...
void checkandgetarguments(int argc, char **argv, int *nx, int *ny, int *xprocs, int *yprocs, int *nxp, int *nyp, int *barrier, int size, int rank){
  ...
  if ( argc != 7 ){
    /* We print argv[0] assuming it is the program name */
    if(rank == 0){
      printf("usage: %s filename nx ny xprocs yprocs barrier\n", argv[0] );
      printf("This application expects you to provide the input file name, the size of the input data set (nx*ny), the number of processes you want in each dimension (xproc and yproc), and whether to use a barrier before timing (barrier, 0 is no barrier, 1 uses a barrier)\n");
    }
    MPI_Finalize();
    exit(-1);
